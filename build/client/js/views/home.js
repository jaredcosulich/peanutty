...
(function($) {
  ...
  Peanutty = (function() {
    ...
    Peanutty.prototype.runSimulation = function() {
      var update;
      window.requestAnimFrame = (__bind(function() {
    ...
    Peanutty.prototype.destroyDynamicObjects = function() {
      ...
      while (body != null) {
        b = body;
        body = body.m_next;
        _results.push(b.m_type === b2d.Dynamics.b2Body.b2_dynamicBody ? this.world.DestroyBody(b) : void 0);
      }
      return _results;
    };
    ...
    Peanutty.prototype.initCode = function() {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        _results.push(__bind(function(contentEditable) {
          $(contentEditable).bind('keydown', this.handleContentEditableKey);
          $(contentEditable).bind('keyup', this.handleContentEditableKey);
          return $(contentEditable).bind('keypress', this.handleContentEditableKey);
  ...
  Peanutty.runCode = __bind(function(code) {
  ...
  Peanutty.runScript = __bind(function(script) {
    if (script == null) {
      script = view.$('#codes .script');
    }
    return Peanutty.runCode(script);
  }, this);
  Peanutty.setStage = __bind(function(stage) {
    if (stage == null) {
      stage = view.$('#codes .stage');
    }
    return Peanutty.runCode(stage);
  }, this);
  Peanutty.loadEnvironment = __bind(function(environment) {
    if (environment == null) {
      environment = view.$('#codes .environment');
    }
  ...
  views.Home = (function() {
    ...
    Home.prototype.renderView = function() {
      ...
      this.$('#tabs .tab').bind('click', __bind(function(e) {
        var tab;
        $('#tabs .tab').removeClass('selected');
        tab = $(e.currentTarget);
        tab.addClass('selected');
        $('#codes .code').removeClass('selected');
        return this.$("#codes ." + (tab[0].className.replace('tab', '').replace('selected', '').replace(/\s/ig, ''))).addClass('selected');
      }, this));
      this.$('#execute .run_script').bind('click', __bind(function(e) {
        peanutty.destroyWorld();
        return Peanutty.runScript();
      }, this));
      this.$('#execute .reset_script').bind('click', __bind(function(e) {
        peanutty.destroyWorld();
        this.loadCode();
        return Peanutty.runScript();
      }, this));
